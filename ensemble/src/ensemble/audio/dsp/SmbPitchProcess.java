...
public class SmbPitchProcess {
	...
	*****************************************************************************/
        	...
        	public static void smbPitchShift(float pitchShift, int numSampsToProcess, int fftFrameSize, int osamp, float sampleRate, double[] indataDouble, double[] outdataDouble)
	        {
	        	...
	        	float[] outdata = new float[indataDouble.length];
	        	for(int i =0;i<indataDouble.length;i++){
	        		indata[i] = (float) indataDouble[i];
	        	}
	            ...
	            for (i = 0; i < numSampsToProcess; i++)
	            {
	                ...
	                if (gRover >= fftFrameSize)
	                {
	                    ...
	                    Arrays.copyOfRange(gOutputAccum, sourceOffset, sourceOffset+fftFrameSize);
	            ...
	            for(int j =0;j<indataDouble.length;j++){
	            	outdataDouble[j] = (double)outdata[j];
	        	}
	        }
